...
/* ─── MEMBER DETAIL MODAL ─── */
const MemberDetailModal = ({ member, onClose, onSendSMS, sending, onDelete, deleting }) => {
  ...
  const fields = [
    ...
    { label: "Date of Birth",     value: member.dateOfBirth ? new Date(member.dateOfBirth).toLocaleDateString("en-NG", { day: "numeric", month: "long", year: "numeric" }) : "—" },
    { label: "Address",           value: member.address || "—" },
    { label: "Occupation",        value: member.occupation || "—" },
    { label: "Services Attended", value: member.numberOfServices !== undefined ? `${member.numberOfServices} service${member.numberOfServices !== 1 ? "s" : ""}` : "—" },
    { label: "Member Since",      value: member.createdAt ? new Date(member.createdAt).toLocaleDateString("en-NG", { day: "numeric", month: "long", year: "numeric" }) : "—" },
  ];
  ...
  return (
    ...
    >
      <div
        style={{ background: "#fff", borderRadius: 20, padding: 28, maxWidth: 520, width: "100%", maxHeight: "88vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: 20, color: colors.deepNavy, fontFamily: "'Georgia', serif", marginBottom: 5 }}>
          ...
          <button
            onClick={onClose}
            style={{ background: "#f1f5f9", border: "none", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontWeight: 700, fontSize: 18, color: "#64748b", flexShrink: 0 }}
          >✕</button>
        </div>

        {/* Details grid */}
        <div style={{ borderRadius: 12, overflow: "hidden", border: "1.5px solid #e8eaf0", marginBottom: 20 }}>
          {fields.map((f, i) => (
            <div key={f.label} style={{ display: "grid", gridTemplateColumns: "145px 1fr", padding: "11px 16px", background: i % 2 === 0 ? "#f8fafc" : "#fff", borderBottom: i < fields.length - 1 ? "1px solid #f1f5f9" : "none" }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.4, paddingTop: 2 }}>{f.label}</span>
              <span style={{ fontSize: 14, fontWeight: 600, color: colors.deepNavy, wordBreak: "break-word" }}>{f.value}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button
            ...
            style={{ flex: 1, padding: "11px 0", borderRadius: 10, fontWeight: 700, fontSize: 14, border: "none", cursor: member.phone && !sending ? "pointer" : "not-allowed", background: member.phone && !sending ? colors.deepNavy : "#e2e8f0", color: member.phone && !sending ? colors.gold : "#94a3b8" }}
          >
            {sending ? "Sending…" : "📱 Send SMS"}
          </button>
          <button
            ...
            style={{ padding: "11px 18px", borderRadius: 10, fontWeight: 700, fontSize: 14, border: "none", cursor: deleting ? "not-allowed" : "pointer", background: deleting ? "#e2e8f0" : "#FEF2F2", color: deleting ? "#94a3b8" : "#DC2626", display: "flex", alignItems: "center", gap: 6 }}
          >
            {deleting ? <><Spinner size={14} color="#94a3b8" /><span>Deleting…</span></> : "🗑 Delete"}
          </button>
        </div>
      </div>
    </div>
...
/* ─── ALL MEMBERS TAB (extracted for useState support) ─── */
const AllMembersTab = ({ members, regularMembers, firstTimers, newConverts, filtered, sendFollowUp, sendingStates, deleteMember, deletingStates, searchTerm, setSearchTerm }) => {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 16, padding: 28, border: "1.5px solid #e8eaf0", boxShadow: "0 2px 12px rgba(11,27,63,0.06)" }}>
      <SectionHeader
        ...
        subtitle={`${regularMembers.length} Regular · ${firstTimers.length} First Timers · ${newConverts.length} New Converts`}
      />
      <div style={{ marginBottom: 12, padding: "10px 14px", background: `${colors.deepNavy}06`, border: `1px solid ${colors.deepNavy}15`, borderRadius: 10, fontSize: 12, color: "#475569" }}>
        💡 <strong style={{ color: colors.deepNavy }}>Tip:</strong> Click any member card to view their full profile details.
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 14 }}>
        {filtered.map(m => (
          <div
            key={m._id}
            onClick={e => { if (e.target.closest("button")) return; setSelectedMember(m); }}
            style={{ cursor: "pointer", position: "relative" }}
          >
            {/* "View details" hover hint */}
            <div style={{ position: "absolute", top: 10, right: 10, zIndex: 2, fontSize: 10, fontWeight: 700, color: colors.deepNavy, background: colors.gold, borderRadius: 6, padding: "2px 7px", opacity: 0.85, pointerEvents: "none" }}>
              👁 View
            </div>
            <MemberCard
              member={m}
              onSendSMS={sendFollowUp}
              sending={sendingStates[m._id]}
              onDelete={deleteMember}
              deleting={deletingStates[m._id]}
            />
          </div>
        ))}
        {filtered.length === 0 && (
          <div style={{ gridColumn: "1/-1", textAlign: "center", padding: 48, color: "#94a3b8" }}>No members found</div>
        )}
      </div>
      ...
      {selectedMember && (
        <MemberDetailModal
          ...
          onSendSMS={sendFollowUp}
          sending={sendingStates[selectedMember._id]}
          onDelete={deleteMember}
          deleting={deletingStates[selectedMember._id]}
        />
      )}
    </div>
  );
...
const AdminDashboard = () => {
  ...
  const [counsellingFilter, setCounsellingFilter] = useState("all");
  ...
  const tabs = [
    ...
    { id: "counselling", label: "Counselling",        icon: "🕊️" },
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 100, background: colors.deepNavy, boxShadow: "0 4px 24px rgba(11,27,63,0.35)", padding: "0 28px", borderBottom: "1px solid rgba(255,215,0,0.1)" }}>
        <div style={{ maxWidth: 1340, margin: "0 auto", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: "rgba(255,215,0,0.12)", border: "1.5px solid rgba(255,215,0,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>⛪</div>
            <div>
      ...
      <div style={{ maxWidth: 1340, margin: "0 auto", padding: "28px 20px" }}>
        ...
        {activeTab === "counselling" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{ background: "#fff", borderRadius: 16, padding: 28, border: "1.5px solid #e8eaf0", boxShadow: "0 2px 12px rgba(11,27,63,0.06)" }}>
              <SectionHeader title="🕊️ Counselling Bookings" count={counsellingFiltered.length} subtitle="Manage all session requests from the website" />
                        ...
                        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 12, color: "#64748b", marginTop: 5, display: "flex", gap: 14, flexWrap: "wrap" }}>
                              {c.phone && <span>📱 {c.phone}</span>}
                              {c.email && <span>✉️ {c.email}</span>}
                              {c.date && <span>📅 {new Date(c.date).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</span>}
                              {c.createdAt && <span style={{ color: "#94a3b8" }}>Submitted: {new Date(c.createdAt).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</span>}
                            </div>
                          </div>
                          ...
                          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                            {c.status !== "Confirmed" && c.status !== "Completed" && (
                              <button onClick={() => updateCounsellingStatus(c._id, "Confirmed")}
                                style={{ padding: "7px 16px", borderRadius: 8, border: "1px solid #BFDBFE", background: "#EFF6FF", color: "#1E40AF", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                                ✅ Confirm
                              </button>
                            )}
                            {c.status !== "Completed" && (
                              <button onClick={() => updateCounsellingStatus(c._id, "Completed")}
                                style={{ padding: "7px 16px", borderRadius: 8, border: "1px solid #86EFAC", background: "#F0FDF4", color: "#15803D", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                                🎉 Mark Done
                              </button>
                            )}
                            <button onClick={() => updateCounsellingStatus(c._id, "Cancelled")}
                              style={{ padding: "7px 16px", borderRadius: 8, border: "1px solid #FCA5A5", background: "#FEF2F2", color: "#B91C1C", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                              ✕ Cancel
                            </button>
                          </div>
                        )}
