...
// const CounselStatusBadge = ({ status }) => {
//   const s = COUNSEL_STATUS_STYLE[status] || COUNSEL_STATUS_STYLE.Pending;
//   return (
//     <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, background: s.bg, color: s.color, flexShrink: 0 }}>
...
//    COUNSELLING TAB
// ───────────────────────────────────────── */
// const CounsellingTab = ({ sessions, updatingStates, deletingStates, onUpdateStatus, onDelete }) => {
//   const [filter, setFilter]         = useState("all");
//   const [typeFilter, setTypeFilter] = useState("all");
//   const [searchTerm, setSearchTerm] = useState("");

//   const STATUS_FILTERS = [
//     { key: "all",       label: "All",          value: sessions.length,                                        color: colors.deepNavy, bg: "#EEF2FF" },
//     { key: "Pending",   label: "⏳ Pending",   value: sessions.filter(s => s.status === "Pending").length,   color: "#92400E",       bg: "#FFFBEB" },
//     { key: "Confirmed", label: "✅ Confirmed",  value: sessions.filter(s => s.status === "Confirmed").length, color: "#1D4ED8",       bg: "#EFF6FF" },
//     { key: "Completed", label: "🎉 Completed",  value: sessions.filter(s => s.status === "Completed").length, color: "#15803D",       bg: "#F0FDF4" },
//     { key: "Cancelled", label: "❌ Cancelled",  value: sessions.filter(s => s.status === "Cancelled").length, color: "#B91C1C",       bg: "#FEF2F2" },
//   ];
...
//       {pending > 0 && (
//         <div style={{ padding: "14px 20px", borderRadius: 12, background: "#FFFBEB", border: "1.5px solid #FDE68A", display: "flex", alignItems: "center", gap: 12 }}>
//           <span style={{ fontSize: 20 }}>⏳</span>
//           <div>
//             <div style={{ fontWeight: 800, fontSize: 14, color: "#92400E" }}>{pending} session{pending > 1 ? "s" : ""} awaiting confirmation</div>
//             <div style={{ fontSize: 12, color: "#92400E", opacity: 0.8, marginTop: 2 }}>Review and confirm or cancel pending counselling bookings below.</div>
//           </div>
//         </div>
//       )}
//       {/* Status filter chips */}
...
//           <button key={f.key} onClick={() => setFilter(f.key)} style={{ padding: "12px 10px", borderRadius: 12, border: `2px solid ${filter === f.key ? f.color : "transparent"}`, background: filter === f.key ? f.bg : "#fff", cursor: "pointer", textAlign: "center", transition: "all 0.15s", boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
//             <div style={{ fontSize: 22, fontWeight: 800, color: f.color }}>{f.value}</div>
//             <div style={{ fontSize: 11, fontWeight: 700, color: f.color, marginTop: 2 }}>{f.label}</div>
//           </button>
//         ))}
//       </div>
//       {/* Main panel */}
...
//           <SectionHeader title="🤝 Counselling Sessions" count={displayed.length} subtitle="Manage all counselling booking requests" />
//           <div style={{ display: "flex", gap: 8 }}>
//             {["all", "Online", "Physical"].map(t => (
//               <button key={t} onClick={() => setTypeFilter(t)} style={{ padding: "6px 14px", borderRadius: 20, border: "none", fontWeight: 700, fontSize: 12, cursor: "pointer", background: typeFilter === t ? colors.deepNavy : "#f1f5f9", color: typeFilter === t ? colors.gold : "#475569", transition: "all 0.15s" }}>
//                 {t === "all" ? "All Types" : t === "Online" ? "🌐 Online" : "🏢 Physical"}
//               </button>
//             ))}
//           </div>
//         </div>
...
//           <div style={{ textAlign: "center", padding: "48px 0", color: "#94a3b8" }}>
//             <div style={{ fontSize: 40, marginBottom: 12 }}>🤝</div>
//             <div style={{ fontWeight: 700, fontSize: 15 }}>{sessions.length === 0 ? "No counselling sessions booked yet" : "No sessions match this filter"}</div>
//           </div>
//         ) : (
//           <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
//             {displayed.map(s => {
//               const sd = new Date(s.date);
//               const isUpcoming = sd >= new Date();
//               const borderColor = s.status === "Pending" ? "#FDE68A" : s.status === "Confirmed" ? "#93C5FD" : s.status === "Completed" ? "#86EFAC" : "#FCA5A5";
//               return (
//                 <div key={s._id} style={{ padding: "18px 22px", borderRadius: 14, border: `1.5px solid ${borderColor}`, background: "#fafafa", display: "flex", flexDirection: "column", gap: 12, transition: "box-shadow 0.2s" }}
//                   onMouseEnter={e => e.currentTarget.style.boxShadow = "0 4px 16px rgba(11,27,63,0.1)"}
//                   onMouseLeave={e => e.currentTarget.style.boxShadow = "none"}>
//                   {/* Top row */}
//                   <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
//                     <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
//                       <div style={{ background: colors.deepNavy, color: colors.gold, borderRadius: 10, padding: "8px 12px", textAlign: "center", minWidth: 52, flexShrink: 0 }}>
//                         <div style={{ fontSize: 10, fontWeight: 600, opacity: 0.8 }}>{MONTHS_SHORT[sd.getMonth()]}</div>
//                         <div style={{ fontSize: 22, fontWeight: 800, lineHeight: 1 }}>{sd.getDate()}</div>
//                         <div style={{ fontSize: 10, fontWeight: 600, opacity: 0.8 }}>{sd.getFullYear()}</div>
//                       </div>
//                       <div>
//                         <div style={{ fontWeight: 800, fontSize: 16, color: colors.deepNavy }}>{s.name}</div>
//                         <div style={{ fontSize: 12, color: "#64748b", marginTop: 3, display: "flex", gap: 12, flexWrap: "wrap" }}>
//                           {s.phone && <span>📱 {s.phone}</span>}
//                           {s.email && <span>✉️ {s.email}</span>}
//                         </div>
//                         <div style={{ display: "flex", gap: 8, marginTop: 6, flexWrap: "wrap", alignItems: "center" }}>
//                           <CounselStatusBadge status={s.status} />
//                           <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, background: s.sessionType === "Online" ? "#EFF6FF" : "#F5F3FF", color: s.sessionType === "Online" ? "#1D4ED8" : "#6D28D9" }}>
//                             {s.sessionType === "Online" ? "🌐 Online" : "🏢 Physical"}
//                           </span>
//                           {isUpcoming && !["Cancelled","Completed"].includes(s.status) && (
//                             <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, background: "#F0FDF4", color: "#15803D" }}>📅 Upcoming</span>
//                           )}
//                         </div>
//                       </div>
//                     </div>
//                     <button onClick={() => onDelete(s._id)} disabled={deletingStates[s._id]}
//                       style={{ padding: "6px 12px", borderRadius: 8, border: "none", background: deletingStates[s._id] ? "#e2e8f0" : "#FEF2F2", color: deletingStates[s._id] ? "#94a3b8" : "#DC2626", fontWeight: 700, fontSize: 12, cursor: deletingStates[s._id] ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
//                       {deletingStates[s._id] ? <><Spinner size={12} color="#94a3b8" /><span>Deleting…</span></> : "🗑 Delete"}
//                     </button>
//                   </div>
//                   {/* Message */}
//                   {s.message && (
//                     <div style={{ padding: "10px 14px", background: "#fff", borderRadius: 10, border: "1.5px solid #e8eaf0", fontSize: 13, color: "#475569", lineHeight: 1.6, fontStyle: "italic" }}>
//                       💬 "{s.message}"
//                     </div>
//                   )}
//                   {/* Action buttons */}
...
//                     <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
//                       {s.status === "Pending" && (
//                         <button onClick={() => onUpdateStatus(s._id, "Confirmed")} disabled={updatingStates[s._id]}
//                           style={{ flex: 1, padding: "8px 0", borderRadius: 9, border: "none", fontWeight: 700, fontSize: 13, cursor: updatingStates[s._id] ? "not-allowed" : "pointer", background: updatingStates[s._id] ? "#e2e8f0" : "#2563EB", color: updatingStates[s._id] ? "#94a3b8" : "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
//                           {updatingStates[s._id] ? <><Spinner size={13} color="#94a3b8" /><span>Updating…</span></> : "✅ Confirm Session"}
//                         </button>
//                       )}
//                       {s.status === "Confirmed" && (
//                         <button onClick={() => onUpdateStatus(s._id, "Completed")} disabled={updatingStates[s._id]}
//                           style={{ flex: 1, padding: "8px 0", borderRadius: 9, border: "none", fontWeight: 700, fontSize: 13, cursor: updatingStates[s._id] ? "not-allowed" : "pointer", background: updatingStates[s._id] ? "#e2e8f0" : "#16A34A", color: updatingStates[s._id] ? "#94a3b8" : "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
//                           {updatingStates[s._id] ? <><Spinner size={13} color="#94a3b8" /><span>Updating…</span></> : "🎉 Mark Completed"}
//                         </button>
//                       )}
//                       <button onClick={() => onUpdateStatus(s._id, "Cancelled")} disabled={updatingStates[s._id]}
//                         style={{ padding: "8px 16px", borderRadius: 9, border: "1.5px solid #FCA5A5", fontWeight: 700, fontSize: 13, cursor: updatingStates[s._id] ? "not-allowed" : "pointer", background: "#fff", color: "#DC2626" }}>
//                         Cancel
//                       </button>
//                     </div>
//                   )}
//                   {["Completed","Cancelled"].includes(s.status) && (
//                     <div style={{ padding: "8px 14px", borderRadius: 9, background: s.status === "Completed" ? "#F0FDF4" : "#FEF2F2", border: `1px solid ${s.status === "Completed" ? "#86EFAC" : "#FCA5A5"}`, fontSize: 13, fontWeight: 700, color: s.status === "Completed" ? "#15803D" : "#B91C1C", textAlign: "center" }}>
//                       {s.status === "Completed" ? "🎉 Session completed" : "❌ Session cancelled"}
//                     </div>
//                   )}
//                 </div>
//               );
...
//   const [birthdays, setBirthdays]                     = useState([]);
//   const [contacts, setContacts]                       = useState([]);
//   const [counsellingSessions, setCounsellingSessions] = useState([]);  // ← NEW
//   const [loading, setLoading]                         = useState(true);
//   const [sendingStates, setSendingStates]             = useState({});
//   const [deletingStates, setDeletingStates]           = useState({});
//   const [counselUpdating, setCounselUpdating]         = useState({});  // ← NEW
//   const [counselDeleting, setCounselDeleting]         = useState({});  // ← NEW
...
//     setLoading(true);
//     try {
//       const [membersRes, prayersRes, testimoniesRes, birthdaysRes, contactsRes, counsellingRes] = await Promise.all([
//         axios.get(`${API_URL}/api/members`),
//         axios.get(`${API_URL}/api/prayers`),
//         axios.get(`${API_URL}/api/testimonies`),
//         axios.get(`${API_URL}/api/birthdays/upcoming`),
//         axios.get(`${API_URL}/api/contact`),
//         axios.get(`${API_URL}/api/counselling`),   // ← NEW
//       ]);
//       setMembers(membersRes.data.data || []);
//       setPrayers(prayersRes.data.data || []);
//       setTestimonies(testimoniesRes.data.data || []);
//       setBirthdays(birthdaysRes.data.data || []);
//       setContacts(contactsRes.data.data || []);
//       setCounsellingSessions(counsellingRes.data.data || []);  // ← NEW
//     } catch (err) { console.error("fetchData error:", err); }
//     finally { setLoading(false); }
//   };
...
//   const updateCounsellingStatus = async (id, status) => {
//     setCounselUpdating(p => ({ ...p, [id]: true }));
//     try {
//       await axios.put(`${API_URL}/api/counselling/${id}/status`, { status });
//       setToast(`✅ Session marked as ${status}!`);
//       fetchData();
//     } catch (err) { alert(err.response?.data?.message || "Failed to update status"); }
//     finally { setCounselUpdating(p => ({ ...p, [id]: false })); }
//   };

//   /* ── Counselling: delete ── */
//   const deleteCounsellingSession = async (id) => {
//     if (!window.confirm("Delete this counselling session? This cannot be undone.")) return;
//     setCounselDeleting(p => ({ ...p, [id]: true }));
//     try {
//       await axios.delete(`${API_URL}/api/counselling/${id}`);
//       setToast("✅ Session deleted!");
//       fetchData();
//     } catch (err) { alert(err.response?.data?.message || "Failed to delete session"); }
//     finally { setCounselDeleting(p => ({ ...p, [id]: false })); }
//   };
...
//     if (!window.confirm("Delete this message?")) return;
//     setDeletingStates(p => ({ ...p, [id]: true }));
//     try { await axios.delete(`${API_URL}/api/contact/${id}`); setToast("✅ Message deleted!"); fetchData(); }
//     catch { alert("Failed to delete message"); }
//     finally { setDeletingStates(p => ({ ...p, [id]: false })); }
//   };
...
//   const regularMembers     = members.filter(m => m.category === "Member");
//   const presentCount       = Object.values(attendance).filter(Boolean).length;
//   const pendingCounselling = counsellingSessions.filter(s => s.status === "Pending").length;  // ← NEW
...
//     { id: "birthdays",   label: "Birthdays",          icon: "🎂" },
//     { id: "contacts",    label: "Messages",           icon: "✉️" },
//     { id: "counselling", label: "Counselling",        icon: "🤝" },  // ← NEW
//     { id: "broadcast",   label: "Broadcast",          icon: "📣" },
//   ];
...
//           </div>
//           <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
//             {/* Pending counselling badge in navbar */}
//             {pendingCounselling > 0 && (
//               <button onClick={() => setActiveTab("counselling")} style={{ background: "#F59E0B", color: "#fff", border: "none", borderRadius: 20, padding: "4px 14px", fontWeight: 800, fontSize: 12, cursor: "pointer" }}>
//                 🤝 {pendingCounselling} pending
//               </button>
//             )}
...
//           <StatCard label="Prayer Requests" value={prayers.length}             icon="🙏" />
//           <StatCard label="Upcoming B-days" value={birthdays.length}           icon="🎂" />
//           <StatCard label="Counselling"     value={counsellingSessions.length} icon="🤝" />  {/* ← NEW */}
//           <StatCard label="Messages"        value={contacts.length}            icon="✉️" />
//         </div>
...
//               style={{ padding: "9px 16px", borderRadius: 9, fontWeight: 700, fontSize: 13, border: "none", cursor: "pointer", whiteSpace: "nowrap", background: activeTab === tab.id ? colors.deepNavy : "#fff", color: activeTab === tab.id ? colors.gold : "#475569", boxShadow: activeTab === tab.id ? "0 4px 14px rgba(11,27,63,0.2)" : "0 1px 4px rgba(0,0,0,0.07)", transition: "all 0.18s", position: "relative" }}>
//               {tab.icon} {tab.label}
//               {/* Pending dot indicator on counselling tab */}
//               {tab.id === "counselling" && pendingCounselling > 0 && (
//                 <span style={{ position: "absolute", top: 4, right: 4, width: 8, height: 8, borderRadius: "50%", background: "#F59E0B", border: "2px solid #fff" }} />
//               )}
//             </button>
//           ))}
//         </div>
...
//             updatingStates={counselUpdating}
//             deletingStates={counselDeleting}
//             onUpdateStatus={updateCounsellingStatus}
//             onDelete={deleteCounsellingSession}
//           />
//         )}

//         {/* BROADCAST */}
//         {activeTab === "broadcast" && (
//           <Broadcast members={members} apiUrl={API_URL} onToast={setToast} />
//         )}
